...
object SchemaDefinition {
  ...
  val EpisodeEnum = EnumType(
    "Episode",
    Some("One of the films in the Star Wars Poop"),
    List(
      EnumValue("NEWHOPE",
  ...
  val Query = ObjectType(
    ...
    fields[CharacterRepo, Unit](
      ...
      Field("humans",
            ...
            resolve =
              ctx ⇒ ctx.ctx.getHumans(ctx arg LimitArg, ctx arg OffsetArg)),
      Field("droids",
            ListType(Droid),
            arguments = LimitArg :: OffsetArg :: Nil,
            resolve =
              ctx ⇒ ctx.ctx.getDroids(ctx arg LimitArg, ctx arg OffsetArg))
    )
  )
